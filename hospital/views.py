...
def upload_record(request):
    ...
    # Enforce package constraints for patients
    if user.user_type == 'patient':
        if not user.package:
            return Response({'error': 'No package assigned.'}, status=403)
        # Check max_uploads
        max_uploads = user.package.max_uploads
        if max_uploads == 0:
        ...
        # Check max_storage_mb
        max_storage_mb = user.package.max_storage_mb
        if max_storage_mb > 0:
            total_storage = sum([rec.prescription.size for rec in Record.objects.filter(patient=user, is_deleted=False)]) / (1024 * 1024)
        ...
        # Check max_shares (if sharing is attempted, implement in share endpoint)
        # Check can_share, can_delete, can_set_reminders as needed elsewhere
    # Enforce package for doctors if present (optional, similar logic)
...
def create_reminder(request):
    ...
    if serializer.is_valid():
        ...
        if request.user.user_type == 'doctor' and (not request.user.package or not request.user.package.can_set_reminders):
            return Response({'error': 'Reminders not allowed'}, status=403)
        reminder = serializer.save()
        # Do NOT send email here. Only the management command will send emails when due.
        return Response(serializer.data, status=201)
    return Response(serializer.errors, status=400)
...
def share_record(request, record_id):
    try:
        ...
        # Enforce doctor package share constraints
        if not record.doctor.package.can_share:
            return Response({'error': 'Sharing not allowed for doctor'}, status=403)
        if record.doctor.package.max_shares > 0 and record.shared_with.count() >= record.doctor.package.max_shares:
            return Response({'error': 'Share limit reached for doctor'}, status=403)
        # Enforce patient package share constraints (if patient has a package)
        if record.patient.package and not record.patient.package.can_share:
            return Response({'error': 'Sharing not allowed for patient'}, status=403)
        if record.patient.package and record.patient.package.max_shares > 0 and record.shared_with.count() >= record.patient.package.max_shares:
            return Response({'error': 'Share limit reached for patient'}, status=403)
    ...
    if request.method == 'GET':
        ...
        if serializer.is_valid():
            user = User.objects.create_user(
                ...
                email=serializer.validated_data['email'],
                password=serializer.validated_data.get('password', ''),
                user_type=serializer.validated_data.get('user_type', 'patient'),
                phone_number=serializer.validated_data.get('phone_number', ''),
                package=serializer.validated_data.get('package', None)
            )
...
def download_record(request, record_id):
    try:
        record = Record.objects.get(id=record_id, is_deleted=False)
        # Log for debugging
        logger.info(f"Download attempt: user={request.user.id} ({request.user.user_type}), record.doctor={record.doctor_id}, record.patient={record.patient_id}")
        # Allow download if doctor is uploader or patient is owner (compare by ID)
        if request.user.user_type == 'doctor' and record.doctor_id != request.user.id:
            logger.warning(f"Doctor {request.user.id} denied download for record {record_id}")
            return Response({'error': 'Only the uploading doctor can download'}, status=403)
        if request.user.user_type == 'patient' and record.patient_id != request.user.id:
            logger.warning(f"Patient {request.user.id} denied download for record {record_id}")
            return Response({'error': 'Only the owner patient can download'}, status=403)
        ...
        logger.error(f"Record {record_id} not found for download by user {request.user.id}")
        return Response({'error': 'Record not found'}, status=404)
    except Exception as e:
        logger.error(f"Download error for record {record_id} by user {request.user.id}: {str(e)}")
        return Response({'error': str(e)}, status=500)